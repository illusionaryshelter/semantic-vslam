...
SemanticMapNode::SemanticMapNode()
  ...
  // ---- 发布: 3D 语义地图 + 2D 栅格 (Nav2) + 2D 彩色栅格 (RViz) ----
  ...
  grid_visual_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
      "/semantic_vslam/grid_map_visual", 1);
...
void SemanticMapNode::publishTimer() {
  ...
  // ---- 发布 2D 占据栅格地图 (从 3D 点云投影) ----
  {
    ...
    // ---- 双输出: OccupancyGrid (Nav2) + 彩色扁平点云 (RViz) ----
    ...
    // 2. 彩色扁平点云 — RViz 可视化
    //    每个栅格单元 = 一个 PointXYZRGB 点, z=-0.05 (略低于地面避免裁剪)
    //    颜色 = 该位置代价最高点的原始 RGB
    {
      struct CellInfo {
        int8_t cost = -1;
      ...
      flat_cloud.reserve(width * height);
      const float half = grid_cell_size_ * 0.5f;
      for (int gy = 0; gy < height; ++gy) {
        for (int gx = 0; gx < width; ++gx) {
          const auto &c = cells[gy * width + gx];
          if (c.cost < 0) continue;  // 跳过 unknown
          pcl::PointXYZRGB p;
          p.x = xMin + gx * grid_cell_size_ + half;  // 栅格中心 x
          p.y = yMin + gy * grid_cell_size_ + half;  // 栅格中心 y
          p.z = -0.05f;                               // 略低于地面
          p.r = c.r; p.g = c.g; p.b = c.b;
          flat_cloud.push_back(p);
        }
      }

      sensor_msgs::msg::PointCloud2 vis_msg;
      pcl::toROSMsg(flat_cloud, vis_msg);
      vis_msg.header.stamp = this->now();
      vis_msg.header.frame_id = target_frame_;
      grid_visual_pub_->publish(vis_msg);
    }
  }
